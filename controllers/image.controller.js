...
export function GetImagesByQueriesAndLimit(req, res){
    ...
    if(errorAuth != ""){
        ...
        Image.find(queries).count().then((total) => {
            Image.find(queries, fields).skip(startIndex).limit(limit).sort( { created: -1 } ).then((results) => {
                return res.status(200).json({
                    success: true,
...
export function GetImagesByQueries( req, res){
    ...
    if(errorAuth != ""){
        ...
        Image.find(req.body, fields).then((results) => {
            return res.status(200).json({
                success: true,
    ...
    if(errorAuth != ""){
        ...
        Image.find({name: name}).then(result => {
            ...
            if(!isDuplicate){
                ...
                updateRecord.modifiedBy = "Hung Nguyen";
...
export function UpdateMultiple(req, res) {
    ...
    if(errorAuth != ""){
        ...
        updateRecord.map(x => {
            if(x.name){
                x.slug= convertSlugByName(x.name);
                x.modified = Date.now();
                x.modifiedBy = "Hung Nguyen";
            }
            Image.updateOne({ _id: x.id }, { $set: x }).exec().then().catch((err) => {
                errorMessage += `Can not update ID = ${x.id}, ${err.message} \n`
            });
        });
        res.status(200).json({
            success: true,
            message: 'Images is updated',
